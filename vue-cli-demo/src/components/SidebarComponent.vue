<template>
  <aside class="cart-container">
    <div class="cart">
      <h1 class="cart-title spread">
        <span>
          Cart
          <i class="icofont-cart-alt icofont-1x"></i>
        </span>
        <button @click="toggle" class="cart-close">&times;</button>
      </h1>

      <div class="cart-body">
        <table class="cart-table">
          <thead>
            <tr>
              <th><span class="sr-only">Product Image</span></th>
              <th>Product</th>
              <th>Price</th>
              <th>Qty</th>
              <th>Total</th>
              <th><span class="sr-only">Actions</span></th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(quantity, key, index) in cart" :key="index">
              <td><i class="icofont-carrot icofont-3x"></i></td>
              <td>{{ key }}</td>
              <td>${{ getPrice(key) }}</td>
              <td class="center">{{ quantity }}</td>
              <td>${{ (quantity * getPrice(key)).toFixed(2) }}</td>
              <td class="center">
                <button @click="remove(key)" class="btn btn-light cart-remove">
                  &times;
                </button>
              </td>
            </tr>
          </tbody>
        </table>

        <p v-if="Object.keys(this.cart).length === 0" class="center">
          <em>No items in cart</em>
        </p>
        <div v-if="Object.keys(this.cart).length !== 0" class="spread">
          <span><strong>Total:</strong> \${{ calculateTotal() }}</span>
          <button class="btn btn-light">Checkout</button>
        </div>
      </div>
    </div>
  </aside>
</template>

<script>
export default {
  props: ["toggle", "cart", "inventory", "remove"],
  methods: {
    getPrice(key) {
      return this.inventory.find((item) => item.name === key).price.USD;
    },
    calculateTotal: function () {
      const total = Object.entries(this.cart).reduce((accumulator, current) => {
        return accumulator + current[1] * this.getPrice(current[0]);
      }, 0);
      return total.toFixed(2);
    },
  },
};
</script>
